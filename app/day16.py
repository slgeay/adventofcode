...
### ? doesn't work for 2 players over 10 secondsn why?
def max_water_with_teleportation(valves: dict[str, tuple[int, list[str]]], times: list[int], open_valves: set) -> int:
    ...
    water = valves[best_valve][0] * (best_time - 1) + max_water_with_teleportation(valves, sorted_times[:-1] + [best_time - 2], open_valves)
    open_valves.remove(best_valve)

    return water


def key(t: int, players: list[str], open_valves: set) -> str:
    return str(t) + "".join(players) + "".join(sorted(open_valves))


def max_water(valves: dict[str, tuple[int, list[str]]], t_start: int, t: int, players: list[str], open_valves: set, cache: dict[str, int], teleport: bool, acc: int, maxx: int) -> int:
    ### if all valves are open, we are done
    if len(open_valves) == len([v for v in valves if valves[v][0]]):
        return 0
    ...
    if t < 2:
        return 0 if len(players) == 1 else max_water(valves, t_start, t_start, players[1:], open_valves, cache, teleport, acc, maxx)

    ### if we already know the best solution for this state, return it
    k = key(t, players, open_valves)
    if k in cache:
        return cache[k]

    ### teleportation optimization, no need to check real solutions if we can't do better by cheating
    if teleport and acc + max_water_with_teleportation(valves, [t] + [t_start for _ in players[1:]], open_valves) < maxx:
        return 0
    ...
    if player not in open_valves and valves[player][0]:
        ...
        water = new_water + max_water(valves, t_start, t - 1, players, open_valves, cache, teleport, acc + new_water, maxx)
        open_valves.remove(player)

    ### check the valves connected to the current one
    for valve in valves[player][1]:
        water = max(water, max_water(valves, t_start, t - 1, [valve] + players[1:], open_valves, cache, teleport, acc, max(maxx, acc + water)))
...
def day16(lines: list[str], t_start: int, players: list[str], teleport: bool) -> str:
    ...
    return str(max_water(valves, t_start, t_start, players, set(), {}, teleport, 0, 0))
